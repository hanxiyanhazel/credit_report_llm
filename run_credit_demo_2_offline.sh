...
APP_DIR="$ROOT_DIR/credit_demo_2_offline"
VENDOR_DIR="$APP_DIR/offline_test/vendor"
HOST="${BACKEND_HOST:-127.0.0.1}"
PORT="${BACKEND_PORT:-8000}"
PY_BIN="${PYTHON_BIN:-python3.11}"
DATA_ROOT="${CREDIT_DEMO_2_DATA_ROOT:-$APP_DIR/data}"

if [[ ! -d "$APP_DIR" ]]; then
  echo "credit_demo_2_offline 目录不存在: $APP_DIR" >&2
  exit 1
fi
