#!/usr/bin/env python3
"""Enrich XML standard JSON coded fields by inferring labels from PDF text."""
...
def enrich_field_with_label(field_obj: dict[str, Any], label: str) -> dict[str, Any]:
    updated = copy.deepcopy(field_obj)
    updated["label"] = label
    updated["label_source"] = "pdf_text_inferred"
    updated["interpretation_status"] = "resolved_from_pdf"
    meta = updated.setdefault("value_meta", {})
    extra = meta.setdefault("extra_fields", {})
    extra["label_inference_method"] = "code_to_label_mapping_from_pdf"
    return updated


def enrich_xml_labels(xml_obj: dict[str, Any], code_map: dict[str, dict[str, str]]) -> tuple[dict[str, Any], dict[str, int]]:
    ...
    def process_field(field_code: str, field_obj: dict[str, Any]) -> dict[str, Any]:
        ...
        candidate = code_map.get(field_code, {}).get(code, "")
        if candidate:
            stats["coded_fields_resolved_from_pdf"] += 1
            return enrich_field_with_label(field_obj, candidate)
    ...
    enriched["raw_sections"]["source_summary"]["code_label_enrichment"] = {
        "source": "pdf_text_inferred_mapping",
        "stats": stats,
        "mapped_field_count": len(code_map),
    }
...
def main() -> None:
    ...
    code_map = build_code_label_mapping(xml_obj, pdf_obj, pdf_text)
    enriched, stats = enrich_xml_labels(xml_obj, code_map)

    args.output.parent.mkdir(parents=True, exist_ok=True)
    args.output.write_text(json.dumps(enriched, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    print(json.dumps({"stats": stats, "mapped_fields": sorted(code_map.keys())}, ensure_ascii=False, indent=2))
